...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationName(QStringLiteral("harbour-qqgui"));
    app.setOrganizationName(QStringLiteral("harbour-qqgui"));

    qmlRegisterType<OneBotBridge>("Qqcat", 1, 0, "OneBotBridge");

    qInfo() << "qqgui starting";
    ...
    if (!QFileInfo::exists(qmlMain))
        qmlMain = QStringLiteral("qml/harbour-qqgui.qml");
    ...
    // debug helper: QQGUI_SHOT=/some/dir grabs the window every 3s
    const QString shotDir = QString::fromLocal8Bit(qgetenv("QQGUI_SHOT"));
    if (!shotDir.isEmpty()) {
        QDir().mkpath(shotDir);
